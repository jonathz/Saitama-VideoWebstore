...
let listaGeneral = document.querySelector(".spinner-wrapper");
  ...
  listas() //SERIES
    .then(() => {
      ...
      for (numInicio; numInicio < numFinal; numInicio++) {
        rellenoBusqueda[numInicio] = {
          sinopsis: listaSeries[numInicio].summary,
          episodios: listaSeries[numInicio]._links.self.href,
        };
        //Impresion
        document.querySelector(
          ".listado-general"
        ).innerHTML += `<div class="cajita" index="${numInicio}" categ="series"> 
  ...
  listas() //ANIME
    .then(() => {
      ...
      for (numInicio; numInicio < numFinal; numInicio++) {
        ...
        document.querySelector(
          ".listado-general"
        ).innerHTML += `<div class="cajita" index="${numInicio}" categ="anime">
        ...
        rellenoBusqueda[numInicio] = {
          sinopsis: `Estudio creador: ${listaAnime[numInicio].studios[0].name}`,
        };
      }
      let cajas = document.querySelectorAll(".cajita").length;
      for (let i = 0; i < cajas; i++) {
...
function busqueda() {
  ...
  obtenerContenido(
    `http://api.tvmaze.com/search/shows?q=${produccionBusqueda.value}`
  ).then((data) => {
    rellenoBusqueda = [];
    ...
    for (let i = 0; i < data.length; i++) {
      ...
      document.querySelector(
        ".listado-general"
      ).innerHTML += `<div class="cajita" index="${i}" categ="seriesB"> <div>${data[i].show.name} 
                </div><br> <img class="miniatura" src="${data[i].show.image.medium}" > </div>`;
    }
  });
  obtenerContenido(
    `https://api.jikan.moe/v3/search/anime?q=${produccionBusqueda.value}`
  ).then((data) => {
    for (let i = 0; i < data.results.length - 1; i++) {
      document.querySelector(
        ".listado-general"
      ).innerHTML += `<div class="cajita" index="${rellenoBusqueda.length}"> <div>${data.results[i].title} 
...
async function alDetalle() {
  ...
  rellenoBusqueda[cajaSeleccionada].precioSerie =
    rellenoBusqueda[cajaSeleccionada].precioCapitulo * episodios;

  document.querySelector(
    ".foto-detalle"
  ).innerHTML = `<img class="foto-detalle-mod" src="${this.children[2].src}" > </img>`;
  document.querySelector(".nombre-detalle").innerHTML = `${nombreEnLaCaja}`;
  document.querySelector(
    ".precio-detalle"
  ).innerHTML = `Precio: $${rellenoBusqueda[cajaSeleccionada].precioSerie}`;
  document.querySelector(
    ".descripcion-detalle"
  ).innerHTML = `${rellenoBusqueda[cajaSeleccionada].sinopsis}`;
  document.querySelector(".episodios").innerHTML = `Episodios:${episodios}`;
  datos = {
    nombre: nombreEnLaCaja,
    image_url: this.children[2].src,
    precioCapitulo,
    episodios,
    precioProduccion: precioCapitulo * episodios,
  };

  document
    .querySelector(".boton-detalle")
    .addEventListener("click", compiladorDatos);
}
